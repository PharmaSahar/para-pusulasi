...
try:
    from .experiment_lifecycle_contract import EXPERIMENT_LIFECYCLE_SCHEMA_VERSION
    from .experiment_lifecycle_store import ExperimentLifecycleStore, build_experiment_lifecycle_audit_summary
except ImportError:  # pragma: no cover
    from src.experiment_lifecycle_contract import EXPERIMENT_LIFECYCLE_SCHEMA_VERSION
    from src.experiment_lifecycle_store import ExperimentLifecycleStore, build_experiment_lifecycle_audit_summary


DEFAULT_ARTIFACT_PATH = Path("artifacts/latest/project003_sprint4_experiment_lifecycle_assessment.json")
...
def build_assessment_artifact(
    *,
    repo_root: Path,
    store: ExperimentLifecycleStore,
    generated_at: str,
    test_commands: list[str],
    test_results: dict[str, str],
    final_status: str,
) -> dict[str, Any]:
    summary = build_experiment_lifecycle_audit_summary(store=store)
    ...
    acceptance_matrix = {
        ...
        "deterministic_assignment": "PASS" if not summary["assignment_reproducibility_violations"] else "FAIL",
        "immutable_eligibility_snapshot": "PASS",
        "deterministic_exposure_logging": "PASS",
        "exposure_deduplication": "PASS",
        "assignment_reproducibility": "PASS" if not summary["assignment_reproducibility_violations"] else "FAIL",
        "contamination_detection": "PASS",
        "append_only_store": "PASS" if summary["hash_chain"]["valid"] else "FAIL",
        "replay_projection": "PASS" if not summary["replay_errors"] else "FAIL",
        "deterministic_audit": "PASS",
        "record_only_contamination_policy": "PASS",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run experiment lifecycle audit and emit assessment artifact.")
    parser.add_argument("--lifecycle-path", default="logs/experiment_lifecycle.jsonl")
...
def main(argv: list[str] | None = None) -> int:
    ...
    artifact = build_assessment_artifact(
        ...
        store=ExperimentLifecycleStore(lifecycle_path=Path(args.lifecycle_path)),
