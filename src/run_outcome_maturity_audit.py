...
try:
    from .outcome_maturity_contract import OUTCOME_MATURITY_SCHEMA_VERSION
    from .outcome_maturity_store import OutcomeMaturityStore, build_outcome_maturity_audit_summary
except ImportError:  # pragma: no cover - supports script execution via python src/run_outcome_maturity_audit.py
    from src.outcome_maturity_contract import OUTCOME_MATURITY_SCHEMA_VERSION
    from src.outcome_maturity_store import OutcomeMaturityStore, build_outcome_maturity_audit_summary


DEFAULT_ARTIFACT_PATH = Path("artifacts/latest/project003_sprint3_outcome_maturity_assessment.json")
...
def build_assessment_artifact(
    *,
    repo_root: Path,
    store: OutcomeMaturityStore,
    generated_at: str,
    test_commands: list[str],
    test_results: dict[str, str],
    final_status: str,
) -> dict[str, Any]:
    summary = build_outcome_maturity_audit_summary(store=store)
    ...
    acceptance_matrix = {
        ...
        "kpi_categories": "PASS",
        "outcome_store": "PASS" if summary["hash_chain"]["valid"] else "FAIL",
        "outcome_snapshot": "PASS",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run outcome maturity audit and emit assessment artifact.")
    parser.add_argument("--outcome-path", default="logs/outcome_maturity.jsonl")
...
def main(argv: list[str] | None = None) -> int:
    ...
    artifact = build_assessment_artifact(
        ...
        store=OutcomeMaturityStore(outcome_path=Path(args.outcome_path)),
