...
SHADOW_CONTENT_QUALITY_SCHEMA_VERSION = "v1"
SHADOW_CONTENT_QUALITY_VALIDATOR_VERSION = "v1"
...
def _max_severity(findings: list[ShadowFinding]) -> Literal["low", "medium", "high", "none"]:
    ...
    return sorted((f.severity for f in findings), key=lambda x: order[x])[-1]
...
def _detect_not_priced_in_claims(text: str) -> list[str]:
    patterns = [
        ...
        r"\b(piyasaya\s+hen[üu]z\s+yansimadi)\b",
    ]
...
def _short_starts_mid_sentence(short_script: str) -> bool:
    ...
    if first in {"ve", "ama", "fakat", "bu", "şu", "su", "o", "onlar", "bunu", "bunun"}:
        return True
...
def _short_context_payoff_imbalance(short_script: str) -> tuple[bool, bool]:
    text = str(short_script or "").lower()
    context_markers = sum(text.count(token) for token in ["neden", "arka plan", "sebep", "baglam", "bağlam"])
    payoff_markers = sum(text.count(token) for token in ["sonuc", "çözüm", "cozum", "adim", "kazanc", "özet", "ozet"])
...
def _severity_for_status(status: str) -> Literal["low", "medium", "high"]:
...
def _build_finding(code: str, severity: Literal["low", "medium", "high"], message: str, *, details: dict[str, Any] | None = None) -> ShadowFinding:
    return ShadowFinding(code=code, severity=severity, message=message, details=details or {})
...
class ShadowContentQualityEngine:
    ...
    def __init__(
        ...
        *,
        ...
        recent_rows, malformed_count = load_shadow_results(input_path=self.results_path, limit=250)
        ...
        *,
        ...
        if checkpoint == "generation":
            ...
            for code, matcher in [
                ...
                if hits:
                    findings.append(_build_finding(code, "high", f"{code} matched", details={"matched_patterns": hits}))
            ...
            if pump_hits:
                findings.append(_build_finding("pump_style_title_detection", "high", "pump-style wording in title", details={"matched_patterns": pump_hits}))
            ...
            if mismatch_hits:
                findings.append(_build_finding("ticker_company_mismatch_detection", "high", "ticker/company mismatch signal", details={"matches": mismatch_hits}))
            ...
            if td_score < 0.45:
                findings.append(_build_finding("title_description_consistency", "high", "title/description mismatch", details={"score": round(td_score, 4)}))

            for code, matcher in [
                ("unsupported_financial_claim_description", detect_unsupported_financial_claims),
                ("unverifiable_insider_description", detect_unverifiable_insider_information),
                ("guaranteed_return_description", detect_guaranteed_return_wording),
                ("not_priced_in_description", _detect_not_priced_in_claims),
                ("urgent_trade_pressure_description", _detect_urgent_trade_pressure),
            ]:
                hits = matcher(desc_text)
                all_scores.append(_score_dict(code, 0.0 if hits else 1.0, details={"matched_patterns": hits}))
                if hits:
                    findings.append(_build_finding(code, "high", f"{code} matched", details={"matched_patterns": hits}))
            ...
            if link_hits:
                findings.append(_build_finding("misleading_external_link_context", "medium", "misleading link context detected", details={"matched_patterns": link_hits}))
            ...
            if st_score < 0.4:
                findings.append(_build_finding("script_thumbnail_prompt_consistency", "medium", "script/thumbnail prompt mismatch", details={"score": round(st_score, 4)}))
            ...
            if thumb_text and tt_score < 0.35:
                findings.append(_build_finding("title_thumbnail_text_consistency", "high", "title/thumbnail text mismatch", details={"score": round(tt_score, 4)}))
            ...
            if related_hits:
                findings.append(_build_finding("thumbnail_prompt_relevance", "medium", "thumbnail prompt may be unrelated", details={"matches": related_hits}))

            financial_hits = (
                detect_unsupported_financial_claims(thumb_text)
                + detect_unverifiable_insider_information(thumb_text)
                + detect_guaranteed_return_wording(thumb_text)
                + _detect_not_priced_in_claims(thumb_text)
            )
            all_scores.append(_score_dict("thumbnail_text_financial_safety", 0.0 if financial_hits else 1.0, details={"matched_patterns": financial_hits}))
            if financial_hits:
                findings.append(_build_finding("thumbnail_text_financial_safety", "high", "financial safety wording in thumbnail text", details={"matched_patterns": financial_hits}))
            ...
            if wealth_hits:
                findings.append(_build_finding("thumbnail_wealth_imagery_signal", "medium", "misleading wealth imagery signal", details={"matched_patterns": wealth_hits}))
            ...
            if thumb_text:
                ...
                if len_score < 0.7:
                    findings.append(_build_finding("thumbnail_text_length_convention", "low", "thumbnail text length outside preferred range", details={"length": text_len}))
            ...
            if mismatch_hits:
                findings.append(_build_finding("thumbnail_ticker_company_mismatch", "high", "ticker/company mismatch in thumbnail context", details={"matches": mismatch_hits}))
            ...
            if complete_sentences < 1.0:
                findings.append(_build_finding("shorts_sentence_completeness", "medium", "short may start or end mid-sentence"))
            ...
            if abrupt_start:
                findings.append(_build_finding("shorts_abrupt_beginning", "medium", "short begins abruptly"))
            ...
            if abrupt_end:
                findings.append(_build_finding("shorts_abrupt_ending", "medium", "short ends abruptly"))
            ...
            if missing_context:
                findings.append(_build_finding("shorts_missing_context", "medium", "short may miss context"))
            ...
            if missing_hook:
                findings.append(_build_finding("shorts_hook_quality", "medium", "short hook appears weak"))

            ctx_wo_payoff, payoff_wo_ctx = _short_context_payoff_imbalance(short_text)
            if ctx_wo_payoff:
                findings.append(_build_finding("shorts_context_without_payoff", "low", "short has context but weak payoff"))
            if payoff_wo_ctx:
                findings.append(_build_finding("shorts_payoff_without_context", "low", "short has payoff but weak context"))
            ...
            if st_consistency < 0.2:
                findings.append(_build_finding("shorts_title_content_consistency", "medium", "short title/content mismatch", details={"score": round(st_consistency, 4)}))
            ...
            if duration_outside:
                findings.append(_build_finding("shorts_duration_signal", "medium", "short duration signal outside target range", details={"duration_seconds": short_duration_seconds}))
            ...
            is_dup_short, dup_score = detect_duplicate_text(short_text, [item for item in historical_shorts if item], threshold=0.84)
            all_scores.append(_score_dict("duplicate_shorts_content", 1.0 - dup_score, details={"max_similarity": dup_score}))
            if is_dup_short:
                findings.append(_build_finding("duplicate_shorts_content", "high", "duplicate shorts content signal", details={"max_similarity": dup_score}))
            ...
            if obs["card_recommendation"]["state"] == "not_implemented":
                findings.append(_build_finding("card_recommendation_not_implemented", "low", "card recommendation not implemented"))
            if obs["end_screen_recommendation"]["state"] == "not_implemented":
                findings.append(_build_finding("end_screen_recommendation_not_implemented", "low", "end-screen recommendation not implemented"))
            if obs["playlist_recommendation"]["state"] == "recommendation_absent":
                findings.append(_build_finding("playlist_recommendation_absent", "low", "playlist recommendation absent"))
        else:
