...
class _FakeContent:
    title = "Analytics join test"
    created_at = "2026-07-14T10:00:00"
    script = "Deterministic analytics evidence join script." * 10
    description = "desc"
    tags = ["analytics"]
    category_id = "27"
    niche = "kisisel_finans"
    thumbnail_prompt = "chart"
    hook = "hook"
...
def test_default_off_does_not_write(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.delenv("ANALYTICS_EVIDENCE_JOIN_ENABLED", raising=False)
    path = tmp_path / "analytics_evidence_join.jsonl"
    monkeypatch.setenv("ANALYTICS_EVIDENCE_JOIN_PATH", str(path))
...
def test_enabled_writes_and_keeps_pipeline_output(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("ANALYTICS_EVIDENCE_JOIN_ENABLED", "false")
    baseline = pipeline.run_full_pipeline(topic="x", generate_only=False, channel_cfg=cfg)

    monkeypatch.setenv("ANALYTICS_EVIDENCE_JOIN_ENABLED", "true")
...
def test_storage_failure_is_fail_open(monkeypatch, tmp_path: Path) -> None:
    ...
    import src.analytics_evidence_join as aej

    original_append = aej.AnalyticsEvidenceJoinStore.append

    def _boom(self, row):
        raise RuntimeError("store_down")

    monkeypatch.setattr(aej.AnalyticsEvidenceJoinStore, "append", _boom)
    try:
        result = pipeline.run_full_pipeline(topic="x", generate_only=False, channel_cfg=cfg)
    finally:
        monkeypatch.setattr(aej.AnalyticsEvidenceJoinStore, "append", original_append)
