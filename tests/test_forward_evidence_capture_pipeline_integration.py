...
class _FakeContent:
    title = "Forward evidence testi"
    created_at = "2026-07-13T10:00:00"
    script = "Deterministik ileri kanit zinciri aciklamasi." * 12
    description = "Aciklama"
    tags = ["forward", "evidence"]
    category_id = "27"
    niche = "kisisel_finans"
    thumbnail_prompt = "clean chart"
    hook = "Baslangic"
    next_video_teaser = "devami"
...
def test_default_off_does_not_write(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.delenv("FORWARD_EVIDENCE_CAPTURE_ENABLED", raising=False)
    path = tmp_path / "forward_evidence.jsonl"
    monkeypatch.setenv("FORWARD_EVIDENCE_CAPTURE_PATH", str(path))
...
def test_enabled_writes_stages_and_keeps_pipeline_output(monkeypatch, tmp_path: Path) -> None:
    cfg = _setup(monkeypatch, tmp_path)
    path = tmp_path / "forward_evidence.jsonl"
    monkeypatch.setenv("FORWARD_EVIDENCE_CAPTURE_PATH", str(path))

    monkeypatch.setenv("FORWARD_EVIDENCE_CAPTURE_ENABLED", "false")
    baseline = pipeline.run_full_pipeline(topic="x", generate_only=False, channel_cfg=cfg)

    monkeypatch.setenv("FORWARD_EVIDENCE_CAPTURE_ENABLED", "true")
    ...
    stages = [json.loads(line).get("stage") for line in lines]
    expected = {
        "PLANNING_COMPLETE",
        "BLUEPRINT_FINALIZED",
        "PROMPT_FINALIZED",
...
def test_storage_failure_is_fail_open(monkeypatch, tmp_path: Path) -> None:
    ...
    import src.forward_evidence_capture as fec

    original_append = fec.ForwardEvidenceStore.append

    def _boom(self, row):
        raise RuntimeError("store_down")

    monkeypatch.setattr(fec.ForwardEvidenceStore, "append", _boom)
    try:
        result = pipeline.run_full_pipeline(topic="x", generate_only=False, channel_cfg=cfg)
    finally:
        monkeypatch.setattr(fec.ForwardEvidenceStore, "append", original_append)
