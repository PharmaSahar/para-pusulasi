...
def test_shadow_prompt_candidate_lab_executes(monkeypatch, tmp_path: Path):
...
def test_shadow_prompt_candidate_lab_fail_open(monkeypatch, tmp_path: Path):
    cfg = _setup_pipeline(monkeypatch, tmp_path)
    monkeypatch.setenv("CONTENT_QUALITY_SHADOW_MODE_ENABLED", "true")

    import src.offline_prompt_candidate_generator as lab

    monkeypatch.setattr(
        lab,
        "run_offline_prompt_candidate_lab_and_store",
        lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("candidate lab down")),
    )
