...
def test_shadow_prompt_experiment_executes_and_advisory(monkeypatch, tmp_path: Path):
    ...
    assert result.get("shadow_prompt_experiment", {}).get("advisory_only") is True
    assert result.get("shadow_prompt_experiment", {}).get("pipeline_output_changed") is False
    assert result.get("shadow_prompt_experiment", {}).get("decision", {}).get("decision") == "NO_RUNTIME_CHANGE"



def test_shadow_prompt_experiment_fail_open(monkeypatch, tmp_path: Path):
    cfg = _setup_pipeline(monkeypatch, tmp_path)
    monkeypatch.setenv("CONTENT_QUALITY_SHADOW_MODE_ENABLED", "true")

    import src.shadow_prompt_experiment_framework as framework

    monkeypatch.setattr(
        framework,
        "run_prompt_experiment_and_store",
        lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("experiment down")),
    )
...
def test_pipeline_output_unchanged_shadow_on_off(monkeypatch, tmp_path: Path):
    ...
    base = pipeline.run_full_pipeline(topic="x", generate_only=False, channel_cfg=cfg)
