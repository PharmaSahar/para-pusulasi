...
class _FakeContent:
    title = "Planlama baglantisi"
    created_at = "2026-07-13T10:00:00"
    script = "Planlama ve blueprint baglantisi aciklanir." * 12
    description = "Aciklama"
    tags = ["plan", "lineage"]
    category_id = "27"
    niche = "kisisel_finans"
    thumbnail_prompt = "plan chart"
    hook = "Nasil izlenir"
    next_video_teaser = "sonraki adim"
...
def test_default_off_does_not_write(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.delenv("PLANNING_BLUEPRINT_LINEAGE_EVIDENCE_ENABLED", raising=False)
    path = tmp_path / "planning_lineage.jsonl"
    monkeypatch.setenv("PLANNING_BLUEPRINT_LINEAGE_EVIDENCE_PATH", str(path))
...
def test_enabled_writes_and_keeps_pipeline_output(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("PLANNING_BLUEPRINT_LINEAGE_EVIDENCE_ENABLED", "false")
    baseline = pipeline.run_full_pipeline(topic="x", generate_only=False, channel_cfg=cfg)

    monkeypatch.setenv("PLANNING_BLUEPRINT_LINEAGE_EVIDENCE_ENABLED", "true")
...
def test_storage_failure_is_fail_open(monkeypatch, tmp_path: Path) -> None:
    ...
    import src.planning_blueprint_lineage_evidence as ple

    original_append = ple.PlanningLineageEvidenceStore.append

    def _boom(self, row):
        raise RuntimeError("store_down")

    monkeypatch.setattr(ple.PlanningLineageEvidenceStore, "append", _boom)
    try:
        result = pipeline.run_full_pipeline(topic="x", generate_only=False, channel_cfg=cfg)
    finally:
        monkeypatch.setattr(ple.PlanningLineageEvidenceStore, "append", original_append)
