...
def test_shadow_diagnostic_summary_success(monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    monkeypatch.setattr(scheduler.sys, "argv", ["scheduler.py", "--governance-shadow-diagnostic-summary-now"])
    ...
    assert "Governance shadow diagnostic summary: PASS" in out


def test_shadow_diagnostic_summary_fail_open(monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    monkeypatch.setattr(scheduler.sys, "argv", ["scheduler.py", "--governance-shadow-diagnostic-summary-now"])
    ...
    assert "Governance shadow diagnostic summary: FAIL" in out
    assert "expected_fields_present=FAIL" in out


def test_shadow_diagnostic_summary_deterministic_repeated_execution(monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    monkeypatch.setattr(scheduler, "_build_governance_shadow_readiness_report", lambda **_kwargs: _valid_report())
    first = scheduler.run_governance_shadow_diagnostic_summary_once()
    first_out = capsys.readouterr().out
    second = scheduler.run_governance_shadow_diagnostic_summary_once()
...
def test_shadow_diagnostic_summary_callable_entrypoints(monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    monkeypatch.setattr(scheduler, "_build_governance_shadow_readiness_report", lambda **_kwargs: _valid_report())

    rc = scheduler.run_governance_shadow_diagnostic_summary_once()
    out = capsys.readouterr().out
...
def test_shadow_diagnostic_summary_ordering(monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    monkeypatch.setattr(scheduler, "_build_governance_shadow_readiness_report", lambda **_kwargs: _valid_report())
    ...
    assert rc == 0
    assert out.index("- report_entrypoint_callable=PASS") < out.index("- selfcheck_entrypoint_callable=PASS")
    assert out.index("- selfcheck_entrypoint_callable=PASS") < out.index("- contract_validation_entrypoint_callable=PASS")
    assert out.index("- contract_validation_entrypoint_callable=PASS") < out.index("- output_consistency_entrypoint_callable=PASS")


def test_shadow_diagnostic_summary_scheduler_startup_not_invoked(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(scheduler.sys, "argv", ["scheduler.py", "--governance-shadow-diagnostic-summary-now"])
...
def test_shadow_diagnostic_summary_no_operational_coupling(monkeypatch: pytest.MonkeyPatch) -> None:
