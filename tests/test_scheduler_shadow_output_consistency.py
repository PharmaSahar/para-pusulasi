...
def test_shadow_output_consistency_success(monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    monkeypatch.setattr(scheduler.sys, "argv", ["scheduler.py", "--governance-shadow-output-consistency-now"])
    ...
    assert "Governance shadow output consistency: PASS" in out


def test_shadow_output_consistency_fail_open(monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    monkeypatch.setattr(scheduler.sys, "argv", ["scheduler.py", "--governance-shadow-output-consistency-now"])
    ...
    assert "Governance shadow output consistency: FAIL" in out
    assert "expected_fields_present=FAIL" in out


def test_shadow_output_consistency_deterministic_repeated_execution(monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    monkeypatch.setattr(scheduler, "_build_governance_shadow_readiness_report", lambda **_kwargs: _valid_report())
    first = scheduler.run_governance_shadow_output_consistency_once()
    first_out = capsys.readouterr().out
    second = scheduler.run_governance_shadow_output_consistency_once()
...
def test_shadow_output_consistency_callable_entrypoints(monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    monkeypatch.setattr(scheduler, "_build_governance_shadow_readiness_report", lambda **_kwargs: _valid_report())

    rc = scheduler.run_governance_shadow_output_consistency_once()
    out = capsys.readouterr().out
...
def test_shadow_output_consistency_advisory_only_preserved(monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    ...
    assert rc == 1
    assert "advisory_only_true=FAIL" in out


def test_shadow_output_consistency_scheduler_startup_not_invoked(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(scheduler.sys, "argv", ["scheduler.py", "--governance-shadow-output-consistency-now"])
...
def test_shadow_output_consistency_no_operational_coupling(monkeypatch: pytest.MonkeyPatch) -> None:
