...
def test_shadow_selfcheck_success(monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    monkeypatch.setattr(scheduler.sys, "argv", ["scheduler.py", "--governance-shadow-selfcheck-now"])
    ...
    assert "Governance shadow self-check: PASS" in out


def test_shadow_selfcheck_fail_open(monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    monkeypatch.setattr(scheduler.sys, "argv", ["scheduler.py", "--governance-shadow-selfcheck-now"])
    ...
    assert "Governance shadow self-check: FAIL" in out
    assert "expected_fields_present=FAIL" in out


def test_shadow_selfcheck_deterministic_repeated_output(monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    monkeypatch.setattr(scheduler, "_build_governance_shadow_readiness_report", lambda **_kwargs: _valid_report())
    first = scheduler.run_governance_shadow_selfcheck_once()
    first_out = capsys.readouterr().out
    second = scheduler.run_governance_shadow_selfcheck_once()
...
def test_shadow_selfcheck_builder_invoked_once(monkeypatch: pytest.MonkeyPatch) -> None:
    calls = {"count": 0}
    ...
    rc = scheduler.run_governance_shadow_selfcheck_once()

    assert rc == 0
    assert calls["count"] == 1


def test_shadow_selfcheck_expected_field_validation(monkeypatch: pytest.MonkeyPatch, capsys) -> None:
...
def test_shadow_selfcheck_advisory_only_validation(monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    report = _valid_report()
...
def test_shadow_selfcheck_scheduler_startup_not_invoked(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(scheduler.sys, "argv", ["scheduler.py", "--governance-shadow-selfcheck-now"])
...
def test_shadow_selfcheck_no_operational_coupling(monkeypatch: pytest.MonkeyPatch) -> None:
